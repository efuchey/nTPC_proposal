...
\subsection{Contamination from inelastic}
\label{inel_contam}
...
\end{equation}
%
and its statistical error $\Delta F^2 = F^2/\sqrt{N_{QE}}$ are compiled for both kinematics in Table.~V%\ref{tab:Rates}
...
\hline
\end{tabular} 
\caption{Quasi-elastic counting rates, and total form factor (defined in Eq.~11). %~\ref{eq:F2}).
  These rates assume $\Delta t = 12$~hours of running at a beam intensity of $I_{exp} =~30~\mu$A on a liquid deuterium target with length $l_{tgt}~=~15$~cm and density $d_{tgt}~=~0.169~\mathrm{g.cm}^{-3}$}%({\em preliminary})
\label{tab:Rates}
\end{table}
\end{center}
%

The expression of the total form factor $F^2$ is: 
