...
In addition to the beam time, we will also require an estimated 16 hours time (two shifts) between each experimental configuration.
...
\end{itemize}
%
%If this experiment is approved, we plan to insert this experiment in the GMn E12-09-019 run plan in such a way that we can avoid a change in configuration for the SBS magnet and the HCal, which is very time consuming.
Table.~\ref{tab:plan} displays a tentative run plan for this experiment. %, and points out where the nTPE measurement inserts. 
%
\begin{table}[h]
...
%\textcolor{red}{6c see Table.~X%\ref{tab:beamtime}
...
This experiment will take place in Hall C utilizing the BigBite spectrometer to detect electrons scattered off 
